...
2. На машине B в Claude CLI:
   ...
   ```
3. Запуск кодинга на машине A, не переключаясь:
   ```
   > Используй claude_start, чтобы закодить сервлет по docs/spec.md
   ```
   На машине A запустится отдельный Claude CLI; результат заберёшь через `claude_result`.
